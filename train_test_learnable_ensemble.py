...
def main():
    ...
    ############################# TRAINING #######################
    train_logits = np.load(os.path.join(logits_dir, str(args.model_id) + '_' + combo + '_train_logits.npy'))
    train_labels = np.load(os.path.join(logits_dir, str(args.model_id) + '_' + combo + '_train_labels.npy'))
    train_dataset = utility.LogitsDataset(train_logits, train_labels)
    sample_per_class =  utility.count_samples_per_class(train_dataset)
    ...
    val_logits = np.load(os.path.join(logits_dir, str(args.model_id) + '_' + combo + '_val_logits.npy'))
    val_labels = np.load(os.path.join(logits_dir, str(args.model_id) + '_' + combo + '_val_labels.npy'))
    ...
    model_file = os.path.join(os.path.join(ensemble_weights_path, str(args.model_id) + '_' + combo + '.pt'))
    ...
    visualize.plot_history(H, os.path.join(learnable_ensemble_curves, str(args.model_id)+'_'+combo)) 
    ...
    with open(exp_path + '/' + str(args.model_id) + '_' + m + '_learnable_ensemble_training_results.txt', 'w') as file:
    ...
    Y_test = np.load("validation_labels.npy")
    
    test_dataset = utility.NumpyDataset(X_test, Y_test, transform = utility.valTransform)
    ...
    ensemble_model = models.LearnableConvEnsemble(len(model_list))
    ensemble_model.load_state_dict(torch.load(os.path.join(ensemble_weights_path, str(args.model_id) + '_' + combo + '.pt')))

    accuracy, preds, targets = models.evaluation_ensemble(ensemble_model, test)
    print("Test accuracy: ", accuracy)
    visualize.plot_confusion_matrix(targets, preds, class_labels, learnable_ensemble_plots, save = True, name_png = combo, fr='png', title=str(args.model_id) + '_' + combo)
    class_report =  classification_report(targets, preds, labels=class_labels, output_dict=True)


    print('Test accuracy: ' + str(accuracy))
    results = {'accuracy': accuracy}
    eval_list = [results, class_report]
    with open(exp_path + '/' + str(args.model_id) + '_' + combo + '_' + 'learnable_ensemble_metrics.json', 'w+') as file:
